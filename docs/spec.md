...
12. As a 成員, I want 從七個固定分類（食物、衣物、居家、交通、教育、娛樂、其他）中點一個圖示選擇, so that 分類快速一致。
...
- 前端：Vue 3 + Vite 單頁應用，不用 TypeScript，圖表用 Chart.js，日期運算用 date-fns。
...
- Supabase Auth，email + 密碼。後台關閉「Enable Sign Ups」與 email 確認，兩個帳號由建置時手動建立，初始密碼由成員自行保管與更改，不寫入任何檔案。
...
- 兩張表皆啟用 RLS：任何已登入使用者可讀寫全部列（兩人共用一個錢包，不按記錄者隔離）。匿名不可存取。
...
- 一個薄薄的 `api` 模組包住 Supabase：`listAll()`（一次撈全部存款與消費，資料量兩人一輩子也在幾千筆內，不分頁）、`createDeposit / updateDeposit / deleteDeposit`、`createExpense / updateExpense / deleteExpense`。
...
- **紀錄表單**：底部滑出 sheet。頂部「消費 | 存款」切換；欄位：金額（`inputmode=numeric`，自動 focus）、日期（預設選中日）、分類（七個圖示按鈕，消費限定）、內容（消費限定、選填）。編輯模式多「刪除」按鈕，刪除直接執行。
- **分析頁**：區間列（日 / 月 / 年切換 + ‹ › 翻頁）→ 甜甜圈圖（Chart.js doughnut，中心以 plugin 畫總額，外圈標籤為百分比 + 圖示）→ 分類明細列表 → 趨勢柱狀圖（月：31 根以內；年：12 根；日：不渲染）。
- 視覺：淺藍主色（約 `#8EC5E3`）頂欄、白色圓角卡片、消費紅、存款綠、七分類各一固定顏色。手機優先，桌機時內容置中、最大寬度約 480–640px。
